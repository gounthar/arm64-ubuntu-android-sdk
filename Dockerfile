...
RUN apt-get update && apt-get install --no-install-recommends -y software-properties-common && \
   ...
   printf "deb http:// artful main universe" >> /etc/apt/sources.list && apt-get update && \ 
   ...
   apt-get remove -y openjdk-11-jre-headless && yes|$ANDROID_HOME/tools/bin/sdkmanager --licenses && \ 
